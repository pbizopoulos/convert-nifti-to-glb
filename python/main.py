...
def generate_mesh(data, step_size, iterations):
    ...
    if np.unique(volume).size == 2:
        (verts, faces, *_) = marching_cubes(volume, spacing=nifti_object.header.get_zooms(), step_size=step_size)
        ...
        volume[volume == 1] = 0
        volume[volume == 2] = 1
        (verts2, faces2, *_) = marching_cubes(volume, spacing=nifti_object.header.get_zooms(), step_size=step_size)
